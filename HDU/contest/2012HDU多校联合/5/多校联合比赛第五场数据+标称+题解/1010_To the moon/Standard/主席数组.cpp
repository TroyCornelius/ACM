...
#define HCE(it, A) for (typeof(A.rbegin()) it=A.rbegin(); it != A.rend(); ++it)
#define DO(n) while(n--)
...
VI T[N]; VL B[N], C[N];
// Chairman Binary Index Tree
int n, m, a, b, d, tt, _tt;

void _Add(int x, int d){
    LL t = (LL) (n - x - 1) * d;
    while (x) B[x][0] += d, C[x][0] += t, x ^= low_bit(x);
}

void _Add(int l, int r, int d){
    _Add(r, d), _Add(l-1, -d);
}

void Add(int x, int d){
    LL t = (LL) (n - x - 1) * d; int y;
    while (x)  T[x].PB(tt), B[x].PB(*B[x].rbegin() + d), C[x].PB(*C[x].rbegin() + t), x ^= low_bit(x);
}
...
#define loc y = upper_bound(ALL(T[x]), tt) - T[x].begin() - 1

LL Sum(int x){
    LL s1 = 0, s2 = 0, t = n - x; int y; while (x <= n) loc, s1 += B[x][y], s2 += C[x][y], x += low_bit(x);
    return s1 * t - s2;
}
...
int main(){
  ...
  //  freopen("out.txt", "w", stdout);
    //freopen("Poison02.in", "r", stdin);
    //freopen("Poison01.out", "w", stdout);
#endif

    RD(n, m); REP_1(i, n) T[i].PB(0), B[i].PB(0), C[i].PB(0);
    REP_1(i, n) _Add(i, i, RD()); tt = 0;
    ...
    char cmd; DO(m){
        RC(cmd);
         if (cmd == 'B'){
            RD(tt); REP_1(i, n){
                while (*T[i].rbegin() > tt){
                    T[i].pop_back(), B[i].pop_back(), C[i].pop_back();
                }
            }
        }
        else {
            RD(a, b); if (cmd == 'C') ++tt, Add(a, b, RD());
            else if (cmd == 'Q') OT(Sum(a, b));
            else{
                _tt = tt; RD(tt), OT(Sum(a, b)), tt = _tt;
            }
        }
    }
}
