...
#define FOR_C_N(i, a, b) int b____; for (b____=int(b),i=a;i<b____;++i)
#define DWN_C_N(i, b, a) int a____; for (a____=int(a),i=b-1;i>=a____;--i)
...
template<class T0, class T1> inline void RC(T0 &x0, T1 &x1){RC(x0), RC(x1);}
...
struct Operator{
    int l, r, d;
    Operator(){}
    Operator(int _l, int _r, int _d):l(_l), r(_r), d(_d){}
} OP[M];
...
#define ts first.first
#define id first.second
#define ll second.first
#define rr second.second

void solve(int ttt){
    int t = Q.empty() ? -1 : Q.top().ts;
    DWN_N(tt, tt, ttt){
        Add(OP[tt].l, OP[tt].r, -OP[tt].d);
        while (t == tt){
            ans[Q.top().id] = Sum(Q.top().ll, Q.top().rr);
            Q.pop(); t = Q.empty() ? -1 : Q.top().ts;
        }
    }
    ++tt;
}
  ...
  //  freopen("out.txt", "w", stdout);
    ...
    tt = 0, q = 0; char cmd; DO(m){
        RC(cmd); if (cmd == 'B'){
            ...
            RD(a, b); if (cmd == 'C'){
                RD(d); Add(a, b, d);
                OP[tt++] = Operator(a, b, d);
            }
            else if (cmd == 'Q') ans[q++] = Sum(a, b);
            else Q.push(MP(MP(RD(), q++), MP(a, b)));
        }
    }
