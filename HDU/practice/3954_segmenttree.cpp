...
struct segmenttree
{
       ...
       void push_down(int p)
       {
            if (t[p].add)
            {
               if (t[p<<1].le != t[p<<1].ri) t[p<<1].add += t[p].add;
               if (t[p<<1|1].le != t[p<<1|1].ri) t[p<<1|1].add += t[p].add;
               for (int i=k; i>=1 ;i--)
               {
                   if (t[p<<1].num[i] != -1) 
                   {
                     t[p<<1].num[i] += t[p].add * i;
                     if (i < k && t[p<<1].num[i] >= need[i]) level_Up(p<<1, i);
                   }
                   if (t[p<<1|1].num[i] != -1) 
                   {
                     t[p<<1|1].num[i] += t[p].add * i;    
                     if (i < k && t[p<<1|1].num[i] >= need[i]) level_Up(p<<1|1, i);
                   }               
               }
               t[p].add = 0;
            }            
